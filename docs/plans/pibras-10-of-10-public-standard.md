...
- Before Todo 10 can change the RFC state to Accepted: record at least one named maintainer approval with its date and an immutable review reference. While the project has a single maintainer, the gate is reviewability (a reference a third party can check), not headcount; revisit if the maintainer group grows. Missing approval leaves the RFC in Review and blocks Todo 12.
